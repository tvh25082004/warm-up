...
import { BookOpen, Camera, Gamepad2, LogOut, FileUp, Video, Mic, ClipboardList, TrendingUp, History, PenLine, Award } from 'lucide-react';
import {
      ...
      <main className="dashboard-main">
        ...
        {isStudent ? (
          ...
          <motion.div
            className="option-card builder-card glass-panel"
            whileHover={{ scale: 1.03, y: -5 }}
            onClick={() => navigate('/builder/speaking-grade')}
          >
            <div className="icon-wrapper" style={{ background: 'linear-gradient(135deg,#EC4899,#6366F1)' }}>
              <Award size={40} color="white" />
            </div>
            <h3>Chấm IELTS Speaking</h3>
            <p>Upload video hoặc audio: speech-to-text (Whisper) và chấm Speaking theo 4 tiêu chí IELTS (GPT). Lưu các bài đã chấm.</p>
          </motion.div>
            </div>
          </>
        )}
